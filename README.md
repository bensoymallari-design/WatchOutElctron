...
`release/WatchJhon-Setup-7.8.27.exe`

Double-click it. It adds **WatchJhon** to the Start Menu and desktop. That `.exe` is what you copy to other PCs. The splash screen must say **PRODUCER 7.8.27**. The header also shows **7.8.27**. Do not stash `package.json` before the build — that keeps the old splash version.
...
6. **OBS DistroAV NDI:** DistroAV 6.2 already loaded NDI 6.3 — you do **not** need NDI Tools. Connect `HPVS-BPXL-12 (QUBITNDI)`. **Program must be a camera or Color Source.** Display Capture of WatchJhon is a loop: Stage stays on NDI PROGRAM rings (amber asset dot). Splash **7.8.27**.
