...
`release/WatchJhon-Setup-7.8.28.exe`

Double-click it. It adds **WatchJhon** to the Start Menu and desktop. That `.exe` is what you copy to other PCs. The splash screen must say **PRODUCER 7.8.28**. The header also shows **7.8.28**. Do not stash `package.json` before the build — that keeps the old splash version.
...
6. **OBS DistroAV / Resolume NDI:** DistroAV 6.2 already loaded NDI 6.3 — you do **not** need NDI Tools. Connect `HPVS-BPXL-12 (QUBITNDI)`. **Program must be a camera or Color Source.** Display Capture of WatchJhon is a loop. Output uses the **sender’s raster** (Resolume 1920×1080 stays 1920×1080 on the wall; a 5760×1080 composition stays 5760×1080). Splash **7.8.28**.
