...
6. **OBS / Resolume NDI:** turn the NDI output on in that app. **Assets → NDI** lists every source on the LAN (same idea as Resolume). Click **Connect** on the one you want. Chromium cannot decode NDI itself — WatchJhon uses the **NDI Runtime** DLL already on the PC if Resolume or OBS-NDI is installed. You do **not** need NDI Webcam Input. If the dialog says Runtime not found, install the free [NDI Runtime](https://ndi.video/tools/ndi-runtime/) and restart WatchJhon. Do not NDI a **Display Capture of WatchJhon** — Stage would only show WatchJhon looking at itself. NDI a camera or a different scene.
