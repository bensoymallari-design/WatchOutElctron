...
`release/WatchJhon-Setup-7.8.25.exe`

Double-click it. It adds **WatchJhon** to the Start Menu and desktop. That `.exe` is what you copy to other PCs. The splash screen must say **PRODUCER 7.8.25**. The header also shows **7.8.25**. Do not stash `package.json` before the build — that keeps the old splash version.
...
6. **OBS DistroAV NDI:** DistroAV 6.2 already loaded NDI 6.3 — you do **not** need NDI Tools. Connect `HPVS-BPXL-12 (QUBITNDI)`. Put a camera or Color Source on Program (not Display Capture of WatchJhon). If Connect says `ERR_MODULE_NOT_FOUND` / `ndiPixels-*.js`, rebuild **7.8.25** (the helper was running a half-unpacked script). Splash **7.8.25**.
