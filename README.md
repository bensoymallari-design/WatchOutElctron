...
`release/WatchJhon-Setup-7.8.20.exe`

Double-click it. It adds **WatchJhon** to the Start Menu and desktop. That `.exe` is what you copy to other PCs. The splash screen must say **PRODUCER 7.8.20**. The header also shows **7.8.20**. Do not stash `package.json` before the build — that keeps the old splash version.
...
6. **OBS / Resolume NDI:** you do **not** need NDI Tools. Turn on OBS **Tools → NDI → Output Settings → Main Output**. **Assets → NDI** lists every source on the LAN. Click **Connect**. Chromium cannot decode NDI itself — WatchJhon uses the **NDI Runtime** DLL already on the PC if Resolume or OBS-NDI is installed. If the dialog says Runtime not found, install only the free [NDI Runtime](https://ndi.video/tools/ndi-runtime/) (not NDI Tools) and restart WatchJhon. Do not NDI a **Display Capture of WatchJhon** — Stage would only show WatchJhon looking at itself. NDI a camera or a different scene. The asset dot is amber until a picture paints, then green.
